...
import { BookOpenText, ArrowUp, ChevronRight, Play, X, Video, Download } from 'lucide-react';
...
export default function PublicDocumentationPage() {
  ...
  return (
    <div className="bg-white">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3 mt-8">
            ...
            <button
              ...
              className="px-6 py-3 bg-white/20 border-2 border-white/50 text-white font-bold text-sm rounded-full hover:bg-white/30 transition-colors inline-flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Download size={15} />
              ...
              components={{
                ...
                h2: ({ children }) => {
                  const id = typeof children === 'string'
                    ...
                    : undefined;
                  return (
                    <h2 id={id} className="text-2xl font-bold text-gray-900 mt-10 mb-4 pb-2 border-b border-gray-200 scroll-mt-24">
                      {children}
                    </h2>
                  );
                },
                ...
                p: ({ children, node }) => {
                  ...
                  return <p className="text-base text-gray-700 leading-7 mb-4">{children}</p>;
                },
                em: ({ children }) => {
                  const text = typeof children === 'string' ? children : '';
                  if (text.startsWith('Figure ')) {
                    return (
                      <em className="block text-sm text-gray-500 italic mt-2 mb-6 text-center">
                        {children}
                      </em>
                    );
                  }
                ...
                code: ({ className, children }) => {
                  ...
                  if (isBlock) {
                    return (
                      <code className="block bg-gray-50 rounded-lg p-4 text-sm font-mono text-gray-800 overflow-x-auto whitespace-pre my-4">
                        {children}
                      </code>
                    );
                  }
                ...
                img: ({ src, alt }) => (
                  <figure className="my-6">
                    ...
                    {alt && (
                      <figcaption className="text-xs text-gray-500 mt-2 text-center px-2">
                        {alt}
                      </figcaption>
                    )}
                  </figure>
                ...
                blockquote: ({ children }) => (
                  <blockquote className="border-l-4 border-orange-400 pl-4 py-1 my-4 bg-orange-50 rounded-r-lg">
                    {children}
                  </blockquote>
                ),
              }}
      ...
      {/* Scroll to top and Download PDF buttons */}
      {showScrollTop && (
        <div className="fixed bottom-6 right-6 flex flex-col gap-3 z-50">
          <button
            onClick={handleDownloadPdf}
            disabled={isExporting}
            className="p-3 bg-gray-800 text-white rounded-full shadow-lg hover:bg-gray-900 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          <button
            onClick={scrollToTop}
            className="p-3 bg-orange-600 text-white rounded-full shadow-lg hover:bg-orange-700 transition-all"
            title="Scroll to top"
          >
            <ArrowUp size={20} />
          </button>
